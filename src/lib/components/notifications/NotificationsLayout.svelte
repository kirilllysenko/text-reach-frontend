<script lang="ts">
  ...
  import { Notification } from "text-reach-frontend-library";
...
<div
  class="pointer-events-none fixed top-0 left-0 z-999999 flex h-full w-full flex-col items-center justify-end
    gap-2 *:pointer-events-auto sm:gap-5 sm:p-5"
>
  {#each notifications.notifications as notification (notification.id)}
    <div
      class="w-full sm:w-xl"
      role="status"
      onpointerenter={() => notifications.setPaused(true)}
      onpointerleave={() => notifications.setPaused(false)}
