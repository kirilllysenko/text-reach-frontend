import type { ColumnDef, ColumnGroup, ColumnId } from "../column-types";
...
export class ColumnOrderingFeature<TOriginalRow = any> implements IColumnOrderingFeature {
  ...
  private calculateMoveOperation(column: ColumnDef<TOriginalRow>, direction: ColumnMovementDirection): MoveOperation {
    ...
    return isRoot ? this.calculateRootLevelMove(column, direction) : this.calculateGroupLevelMove(column, direction);
  }
  ...
  private calculateGroupExitMove(
    ...
    const parentGroup = currentGroup.parentColumnId ? this.findParentGroupOrThrow(currentGroup.parentColumnId) : null;
  ...
  executeMove(operation: MoveOperation): void {
    ...
    const sourceArray =
      sourceParentId === null ? this.datagrid._columns : this.findParentGroupOrThrow(sourceParentId).columns;

    // Get target array (either root or target group's columns)
    const targetArray =
      targetLocation.parentId === null
  ...
  private wouldCreateCircularReference(
    ...
    while (current) {
      ...
      current = current.parentColumnId ? this.findParentGroupOrThrow(current.parentColumnId) : null;
    }
