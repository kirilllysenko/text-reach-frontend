import type { ColumnDef, ColumnId, LeafColumn, PinningPosition } from "../column-types";
...
export class ColumnPinningFeature implements IColumnPinningFeature {
  ...
  calculateOffset(columns: ColumnDef<any>[], columnId: ColumnId, position: "left" | "right" | null): number {
    ...
    if (position === "left") {
      ...
      return pinnedColumns.slice(0, index).reduce((sum, column) => sum + (column.state.size.width || 0), 0);
    } else {
      // For right-pinned columns, calculate the total width of columns after the target column
      return pinnedColumns.slice(index + 1).reduce((sum, column) => sum + (column.state.size.width || 0), 0);
    }
  }
}
