import type { AggregationFn } from "../column-types";
...
export const aggregationFunctions: {
   ...
   */
  sum: (values: number[]): number => values.reduce((sum, val) => sum + (val || 0), 0),
   ...
   */
  extent: (values: number[]): [number, number] => [Math.min(...values), Math.max(...values)],
   ...
   */
  unique: (values: any[]): any[] => Array.from(new Set(values)),
   ...
   */
  uniqueCount: (values: any[]): number => new Set(values).size,
   ...
   */
  count: (values: any[]): number => values.length,
};
