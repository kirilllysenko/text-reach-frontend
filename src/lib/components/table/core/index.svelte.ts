...
export class DatagridCore<TOriginalRow = any, TMeta = any> {
  ...
  /**
   * Data fields available for operations such as sorting and filtering.
   */
  dataFields: DataFields<TOriginalRow> = new DataFields(this);
   ...
   */
  rowIdGetter: (row: TOriginalRow) => GridRowIdentifier = (row: TOriginalRow) =>
    (row as TOriginalRow & { id: string }).id;
  ...
  initializeGridState(config: DatagridCoreConfig<TOriginalRow>) {
    ...
    this.features = new DatagridFeatures(this, config);
    this.dataFields.initialize(config.dataFields ?? []);
  ...
  private initializeSourceColumns(columns: ColumnDef<TOriginalRow>[], config?: DefaultColumnConfig) {
    ...
    const columnsWithSizes = this.processors.column.applyDefaultColumnSizes(flattenedColumns, config);

    const columnHierarchy = this.processors.column.createColumnHierarchy(columnsWithSizes);

    columns = this.lifecycleHooks.executePreProcessOriginalColumns(columnHierarchy);
...
export class DataFields<TOriginalRow> {
  ...
  initialize(explicitFields: DataField<TOriginalRow>[] = []): void {
    ...
    this.datagrid.columns.getLeafColumns().forEach((column) => {
      if (column.type !== "accessor" && column.type !== "computed") return;

      fields.set(column.columnId, this.createColumnDataField(column));
    });

    explicitFields.forEach((field) => {
      if (explicitFields.filter((current) => current.fieldId === field.fieldId).length > 1) {
        throw new Error(`Duplicate data field ${field.fieldId}`);
      }
  ...
  getAll(): DataField<TOriginalRow>[] {
    return Array.from(this.registry.values());
  }

  findFieldById(fieldId: string): DataField<TOriginalRow> | null {
    return this.registry.get(fieldId) ?? null;
  }

  findFieldByIdOrThrow(fieldId: string): DataField<TOriginalRow> {
...
class Rows<TOriginalRow> implements IRows<TOriginalRow> {
  ...
  getVisibleBasicRows(): GridBasicRow<TOriginalRow>[] {
    return (
      (this.datagrid.cacheManager.rows.filter((row) => !row.isGroupRow()) as GridBasicRow<TOriginalRow>[]) ||
      ([] as GridBasicRow<TOriginalRow>[])
    );
  }
...
class Columns<TOriginalRow> {
  ...
  getColumnsInOrder(): ColumnDef<TOriginalRow>[] {
    ...
    const flatCols = this.datagrid.processors.column.placeGroupColumnsInFront(
    ...
    return [...columns.left, ...this.datagrid.processors.column.createColumnHierarchy(columns.none), ...columns.right];
  }
  ...
  findColumnById(columnId: ColumnId): ColumnDef<TOriginalRow> | null {
    return (
      flattenColumnStructurePreservingGroups(this.datagrid._columns).find((col) => col.columnId === columnId) ?? null
    );
  }
