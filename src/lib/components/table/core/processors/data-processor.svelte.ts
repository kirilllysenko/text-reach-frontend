...
export class DataDataProcessor<TOriginalRow> {
  ...
  executeFullDataTransformation(): void {
    const shouldRunGrouping =
      this.datagrid.features.grouping.activeGroups.length > 0 || this.datagrid.features.grouping.manual;
      ...
      if (this.datagrid.features.columnFaceting.recalculateFacetsAfterFiltering) {
        if (this.datagrid.features.columnFaceting.facetsSource === "originalData") {
          ...
          this.metrics.measure("Column faceting from filtered data", () => {
            this.datagrid.features.columnFaceting.calculateFacets(data, this.datagrid.columns.getLeafColumns());
          });
        }
      }
    } else {
  ...
  applyGlobalSearch(data: TOriginalRow[]): TOriginalRow[] {
    ...
    const applySimpleSearch = (data: TOriginalRow[]) => {
      const searchableColumns = flattenColumnStructureAndClearGroups(this.datagrid._columns)
        .filter((c) => ["accessor", "computed"].includes(c.type))
        .filter((col) => col.options.searchable !== false) as (
      ...
      return data.filter((item) =>
        searchableColumns.some((column) => String(column.getValueFn(item)).toLowerCase().includes(searchValue)),
      );
    };
  ...
  private getAggregationFn(aggregateType: string | { type: string; fn?: AggregationFn }): AggregationFn | null {
    ...
    return aggregationFunctions[aggregateType.type] || this.customAggregationFns.get(aggregateType.type) || null;
  }
  ...
  createHierarchicalData(data: TOriginalRow[]): GridRow<TOriginalRow>[] {
    ...
    const groupByLevel = (rows: TOriginalRow[], depth: number, parentPath = ""): GridRow<TOriginalRow>[] => {
      ...
      rows.forEach((row) => {
        const groupValue = column.getGroupValueFn ? column.getGroupValueFn(row) : column.getValueFn(row);
      ...
      return Array.from(groups.entries()).map(([key, groupRows], index) => {
        const aggregations = flattenColumnStructureAndClearGroups(this.datagrid._columns)
          .filter((col) => (col.type === "accessor" || col.type === "computed") && col.aggregate)
          .flatMap((col) => {
