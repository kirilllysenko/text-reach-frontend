import type { ConversationMessages$result } from "$houdini/artifacts/ConversationMessages";
import type { Conversations$result } from "$houdini/artifacts/Conversations";
import type { MessageDirection$options, MessageStatus$options } from "$houdini/graphql/enums";

type ConversationNode = Conversations$result["conversations"]["edges"][number]["node"];
type MessageNode = ConversationMessages$result["messages"]["edges"][number]["node"];
...
export interface ChatMessage {
  ...
  createdAt: string;
  direction: MessageDirection$options;
  id: string;
  media: MessageNode["media"];
  receivedAt: string | null;
  sentAt: string | null;
  status: MessageStatus$options;
  text: string;
}
