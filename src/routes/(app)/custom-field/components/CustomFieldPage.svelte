<script lang="ts">
  ...
  let table = $state<DatagridCore<CustomFieldViewModel>>(createCustomFieldTable([]));
  ...
  function createCustomFieldTable(data: CustomFieldViewModel[]) {
  ...
  async function reloadRows(): Promise<void> {
    ...
    try {
      ...
      rows = result.rows;
      table = createCustomFieldTable(rows);
    } finally {
      loadingRows = false;
    }
  }
...
<CustomFieldOverlay state={customFieldsState} {sorting} />
