...
import { loadPaymentOverview, paymentBalance, paymentLoading, paymentLoadingError } from "~/lib/state/payment-overview";

export default function PaymentPage() {
  onSettled(() => void loadPaymentOverview());
  return (
    <div class="relative flex h-dvh min-h-0 flex-col gap-3 rounded-2xl bg-gradient-to-br from-slate-100 via-slate-50 to-stone-100 p-2 sm:h-[calc(100dvh-3rem)] sm:p-3">
      ...
      <section class="grid min-h-0 grow grid-cols-1 content-start gap-3 overflow-y-auto rounded-2xl border border-white/70 bg-white/60 p-3 shadow-[0_20px_45px_-25px_rgba(30,41,59,0.45)] backdrop-blur-md lg:grid-cols-[1fr_22rem]">
        <div class="space-y-3">
          <div class="rounded-2xl border border-white/80 bg-white/80 p-5 shadow-sm">
            ...
            <div class="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div class="text-4xl font-semibold text-slate-900 sm:text-5xl">
                {paymentLoading() ? "Loading" : formatPaymentBalance(paymentBalance())}
              </div>
              <div class="flex flex-col gap-2 sm:flex-row">
                <a
                  href={PATH_PAYMENT_TOP_UP}
                  class="flex h-10 items-center justify-center rounded-xl border border-slate-700 bg-slate-700 px-4 text-sm text-white shadow-sm hover:bg-slate-800"
                >
                  Top up
                </a>
                <a
                  href={PATH_PAYMENT_TRANSACTION}
                  class="flex h-10 items-center justify-center rounded-xl border border-white/80 bg-white/90 px-4 text-sm text-slate-700 shadow-sm hover:bg-white"
                >
                  Transactions
                </a>
              </div>
            </div>
            <Show when={paymentLoadingError()}>
              {(error) => (
        ...
        <div class="rounded-2xl border border-white/80 bg-white/80 p-4 shadow-sm">
          <dl class="space-y-4">
            <div>
              ...
              <dd class="mt-1 text-lg font-semibold text-slate-800">{paymentBalance()?.currency ?? "USD"}</dd>
            </div>
            <div>
              ...
              <dd class="mt-1 text-lg font-semibold text-slate-800">
                {paymentLoadingError() ? "Unavailable" : "Ready"}
              </dd>
            </div>
          </dl>
        </div>
